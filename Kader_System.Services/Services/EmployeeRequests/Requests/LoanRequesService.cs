...
namespace Kader_System.Services.Services.EmployeeRequests.Requests
{
    ...
    ) : ILoanRequestService
    {
        ...
        {
            Expression<Func<LoanRequest, bool>> filter = x => x.IsDeleted == false && x.StatuesOfRequest.ApporvalStatus == ((int)model.ApporvalStatus == 0 ? null : (int)model.ApporvalStatus);
